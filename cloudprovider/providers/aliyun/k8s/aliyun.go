...
func (this *aliyunProvider) configureSecurityGroup(d *aliyunecs.Driver) error {
	...
	for _, permission := range perms {
		internetArgs := permission.createAuthorizeSecurityGroupArgs(d.Region, d.SecurityGroupId, ecs.NicTypeInternet)
		if err := getClient(d).AuthorizeSecurityGroup(internetArgs); err != nil {
			return err
		}
		intranetArgs := permission.createAuthorizeSecurityGroupArgs(d.Region, d.SecurityGroupId, ecs.NicTypeIntranet)
		if err := getClient(d).AuthorizeSecurityGroup(intranetArgs); err != nil {
			return err
		}
	}
